...
public:
  ...
  HWControlMessage& set_ricControlMessage(std::string msg, int mgs_len){
            _hw_msg_size = mgs_len;
            for (int i = 0; i < mgs_len; ++i) {
              _hw_msg[i] = msg.c_str()[i];
            }
            _hw_msg[mgs_len] = '\0';

            // fprintf(stderr, "set_ricControlMessage: saved Protobuf message of size %zd\n", _hw_msg_size);
            // for(int i = 0; i < _hw_msg_size; i++) {
            //     fprintf(stderr, " %hhx ", _hw_msg[i]);
            // }
            // fprintf(stderr, "\n");
