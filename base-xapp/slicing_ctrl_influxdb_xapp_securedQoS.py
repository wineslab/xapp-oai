...
## Variables
DUMMY_AI_CTRL = False    # Enable dummy AI control on the slicing
CTRL_FREQ = 20          # Frequency for the slicing control
...
def trigger_slicing_control(sst = 1, sd = 2, min_ration = 20, max_ration = 80):
...
def main():
    ...
    while True:
        ...
        if len(data_sck) <= 0:
            ...
            if report_index == 0:
                control_buf = trigger_slicing_control(sst = 1, sd = 2, min_ration=0, max_ration= 100 )
            if report_index == CTRL_FREQ*2:
                print("+++++++++++ Start Jam Iperf +++++++++++++\n")

            if report_index == CTRL_FREQ*4:
                control_buf = trigger_slicing_control(sst = 1, sd = 2, min_ration=80, max_ration= 100 )
                send_socket(control_sck, control_buf)
                print("Control Buff Sent: min_ration 80 max_ratio 100\n")

            if report_index == CTRL_FREQ*6:
                control_buf = trigger_slicing_control(sst = 1, sd = 2, min_ration=40, max_ration= 100 )
                send_socket(control_sck, control_buf)
                print("Control Buff Sent: min_ration 40 max_ratio 100\n")

            if report_index == CTRL_FREQ*8:
                print("+++++++++++ Stop UE 1 Iperf +++++++++++++\n")

            if report_index == CTRL_FREQ*10:
                print("+++++++++++ Stop UE 2 Iperf +++++++++++++\n")
...
def dummy_data_driven_ctrl(slice_dlth_mapping, ctrl_sock):
    ...
    # Send first control
    control_buf = trigger_slicing_control(sst=sst5, sd=sd5, min_ration=5, max_ration=5)
    ...
    # Send second control
    control_buf = trigger_slicing_control(sst=sst95, sd=sd95, min_ration=10, max_ration=95)
    send_socket(ctrl_sock, control_buf)
