...
#include "e2sm_control.hpp"
 ...
 HWControlMessage::~HWControlMessage(void){
  ...
  mdclog_write(MDCLOG_DEBUG, "Freeing HWControlMessage event trigger object memory");
 ...
 HWControlHeader::~HWControlHeader(void){
  ...
  mdclog_write(MDCLOG_DEBUG, "Freeing HWControlHeader event trigger object memory");
