...
bool HWActionDefinition::encode(unsigned char *buf, size_t *size){
  ...
  // std::cerr << "Printing fields before encoding" << std::endl;
  // RANparameter_List_t test_list = this->_action_defn->choice.actionDefinition_Format1->ranParameter_List;
  // auto **list = test_list.list.array;
  // int countRanParameters = test_list.list.count;
  // for (int z = 0; z < countRanParameters; z++)
  // {
  //   for (int zz = 0; zz < list[z]->ranParameter_Name.size; ++zz) {
  //     std::cerr << list[z]->ranParameter_Name.buf[zz] << std::endl;
  //   }
  // }
  ...
  // E2SM_HelloWorld_ActionDefinition_t *test = (E2SM_HelloWorld_ActionDefinition_t *)calloc(1, sizeof(E2SM_HelloWorld_ActionDefinition_t));
  // ASN_STRUCT_RESET(asn_DEF_E2SM_HelloWorld_ActionDefinition, test);
  // asn_decode(nullptr, ATS_ALIGNED_BASIC_PER, &asn_DEF_E2SM_HelloWorld_ActionDefinition, (void **)&test, buf, *size);
  // xer_fprint(stderr, &asn_DEF_E2SM_HelloWorld_ActionDefinition, test);
  // printf("DEBUG -- End decoded\n");
...
bool HWActionDefinition::setfields(E2SM_HelloWorld_ActionDefinition_t *_action_defn){
 ...
 if(_action_defn == 0){
  ...
  for(RANParamIEs item:*paramList){
    ...
    _ranparam_ie[i].ranParameter_Name.buf = (unsigned char*) calloc(_ranparam_ie[i].ranParameter_Name.size, sizeof(unsigned char));
    memcpy(_ranparam_ie[i].ranParameter_Name.buf, item.get_param_name(), _ranparam_ie[i].ranParameter_Name.size);
    
	  _ranparam_ie[i].ranParameter_Test = item.get_param_test();

	  _ranparam_ie[i].ranParameter_Value.size = item.get_param_value_size();
    _ranparam_ie[i].ranParameter_Value.buf = (unsigned char*) calloc(_ranparam_ie[i].ranParameter_Value.size, sizeof(unsigned char));
    memcpy(_ranparam_ie[i].ranParameter_Value.buf, item.get_param_value(), _ranparam_ie[i].ranParameter_Value.size);
